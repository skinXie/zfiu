<template>
	<div>
		...
		<!-- 数据表格 -->
		<a-table :data-source="list" :loading="loading" :columns="columns" :pagination="pagination"  bordered :rowKey="(record, index) => index" :style="{'max-height': tableHeight + 'px' }" :scroll="{x: 1040, y: tableHeight - 60}">
...
<!--				<a-tooltip slot="packages.packageName" slot-scope="text" placement="topLeft" :title="text">-->
<!--					<span>{{ text }}</span>-->
<!--				</a-tooltip>-->
			</template>
			<template slot="packages" slot-scope="packages">
				<span v-for="(pack,$index) of packages" :key="pack.id">
					{{pack.packageName}}{{$index==packages.length-1?'':','}}
				</span>
			</template>
		...
		<!-- 应用关联-->
		<a-modal v-model="editBindVisible" title="应用关联" @ok="handleBindOk" :maskClosable="false" width="700px">
			<a-form-model ref="editBindForm" :rules="rules" :model="temp" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
				<a-form-model-item label="关联应用" prop="packages">
					<a-select v-model="temp.packages"
										mode="multiple"
										placeholder="点击选择程序">
...
<script>
import {getSshList, editSsh, deleteSsh, installAgentNode,bindPackage} from '../../api/ssh';
...
export default {
	...
	data() {
		return {
			...
			columns: [
				...
				{title: 'Host', dataIndex: 'host', width: 100, ellipsis: true},
				{title: 'Port', dataIndex: 'port', width: 75, ellipsis: true},
				{title: 'User', dataIndex: 'user', width: 100, ellipsis: true},
				{title: '关联应用', dataIndex: 'packages', width: 180, ellipsis: true,scopedSlots: {customRender: 'packages'}},
	...
	methods: {
		...
		handleBind(record) {
			/*let packages = [];
			if(record.packages) {
				record.packages.forEach(function (item) {
					packages.push(item.id);
				})
			}
			this.temp = Object.assign({}, {packages:packages,id:record.id});*/
			this.temp = Object.assign({}, {packages:[],id:record.id});
