	...
	<div>
		...
		<a-table :data-source="list" :loading="loading" :columns="columns" :pagination="false" bordered :rowKey="(record, index) => index" :style="{'max-height': tableHeight + 'px' }" :scroll="{x: 1040, y: tableHeight - 60}">
			<template slot="operation" slot-scope="text, record">
				...
				<a-button type="primary" :disabled="!record.fileDirs" @click="handleFile(record)">文件管理</a-button>
				<a-button type="danger" @click="handleDelete(record)">删除</a-button>
			</template>
		</a-table>
		<!-- 编辑区 -->
		<a-modal v-model="editHostVisible" width="600px" title="编辑主机" @ok="handleEditSshOk" :maskClosable="false">
			<a-form-model ref="editSshForm" :rules="rules" :model="temp" :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }">
				<a-form-model-item label="主机名称" prop="name">
					<a-input v-model="temp.name" placeholder="主机名称"/>
				</a-form-model-item>
...
<script>
import {getHostList, editHost, deleteSsh, installAgentNode} from '../../api/install-host';
import SshFile from '../node/ssh-file';
import Terminal from '../node/terminal';
